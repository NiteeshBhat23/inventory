...
def purchases_csv(days: int = 90, shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
...
def sales_csv(days: int = 90, shop: models.Shop = Depends(get_current_shop), db: Session = Depends(get_db)):
